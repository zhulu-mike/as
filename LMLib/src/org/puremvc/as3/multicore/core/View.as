...
package org.puremvc.as3.multicore.core
{
	...
	public class View implements IView
	{
		...
		public function removeObserver( notificationName:String, notifyContext:Object ):void
		{
			...
			{
				if ( MVCObserver(observers[i]).compareNotifyContext( notifyContext ) == true ) {
		...
		public function registerMediator( mediator:IMediator ) : void
		{
			...
			if ( interests.length > 0) 
			{
				...
				var observer:MVCObserver = new MVCObserver( mediator.handleNotification, mediator );
		...
		{
			if (mediatorMap && mediatorMap.length > 0)
			{
				var key:String;
				for (key in mediatorMap)
				{
					removeMediator(key);
				}
			}
