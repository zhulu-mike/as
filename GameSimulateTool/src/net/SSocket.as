...
//					_protocol = ObjectPoolManager.getInstance().getItem(SProtocol) as SProtocol;
				...
				if (_protocol.type != 20044 && _protocol.type != 20002 && _protocol.type != 20004 && _protocol.type != 20005)
				{
					_msgLen = -1;
					continue;
				}
				_buffer.position = 0;
				if (_protocol.type != 20005){
					DecodeUtil.ByteArrayArichmeticXOR(_buffer,0, _buffer.length - 1);
				}
				_buffer.position = 0;
				var jsonStr:String = _protocol.type != 20005 ? _buffer.readUTFBytes(_buffer.bytesAvailable) : "";
				_protocol.length = _buffer.length;
				//Logger.warn(jsonStr);
				try{
					_protocol.body = _protocol.type != 20005 ? com.adobe.serialization.json.JSON.decode(jsonStr) : {};
				...
				_msgLen = -1;
				trace(_protocol.type+"解析时间："+(getTimer()-ttt));
			}
		}
